        ...
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden" style={{ boxShadow: '0 1px 2px rgba(0,0,0,0.04)' }}>
          <div className="flex items-center justify-between px-5 py-3 border-b border-slate-100">
            ...
            <div className="flex items-center gap-3">
              <span className="text-xs text-slate-400">Showing {sortedResults.length > 0 ? (page - 1) * pageSize + 1 : 0}â€“{Math.min(page * pageSize, sortedResults.length)}</span>
                ...
                {paginatedResults.map(r => {
                  ...
                  return (
                    <tr key={r.id} className={`group transition-colors duration-100 ${isSelected ? 'bg-blue-50' : 'hover:bg-slate-50'}`}>
                      ...
                      <td className="px-4 py-3"><span className={`text-xs font-semibold px-2 py-0.5 rounded ${gradeColor[r.grade || 'F'] || 'text-slate-600 bg-slate-50'}`}>{r.grade || 'â€”'}</span></td>
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div><label className="block text-sm font-medium text-slate-700 mb-1.5">Grade</label><div className={`px-4 py-2.5 border rounded-lg text-sm font-semibold text-center ${gradeColor[formData.grade] || 'text-slate-600 bg-slate-50'}`}>{formData.grade || 'â€”'}</div></div>
              </div>
              ...
              <div className="flex items-start gap-4 p-5 bg-slate-50 rounded-xl border border-slate-200">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap items-center gap-2 mt-1.5">
                    <span className={`text-xs font-bold px-2 py-0.5 rounded ${gradeColor[viewingResult.grade || 'F']}`}>Grade: {viewingResult.grade || 'â€”'}</span>
                  ...
                  <p className="text-xs text-slate-400 mt-1">{viewingResult.percentage}% â€” {viewingResult.marksObtained}/{viewingResult.totalMarks} marks</p>
                </div>
              </div>
