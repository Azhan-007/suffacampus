      ...
      <div className="space-y-8">
        ...
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden" style={{ boxShadow: '0 1px 2px rgba(0,0,0,0.04)' }}>
          <div className="flex items-center justify-between px-5 py-3 border-b border-slate-100">
            ...
            <div className="flex items-center gap-3"><span className="text-xs text-slate-400">Showing {sortedFees.length > 0 ? (page - 1) * pageSize + 1 : 0}â€“{Math.min(page * pageSize, sortedFees.length)}</span><div className="w-[100px]"><Select value={String(pageSize)} onChange={e => { setPageSize(Number(e.target.value)); setPage(1); }} options={PAGE_SIZE_OPTIONS.map(n => ({ value: String(n), label: `${n} rows` }))} /></div></div>
          </div>
                ...
                {paginatedFees.map(f => {
                  ...
                  return (
                    <tr key={f.id} className={`group transition-colors duration-100 ${isSelected ? 'bg-blue-50' : 'hover:bg-slate-50'}`}>
                      ...
                      <td className="px-4 py-3"><span className="text-sm text-slate-600 tabular-nums">{f.amountPaid ? formatCurrency(f.amountPaid) : 'â€”'}</span></td>
            ...
            <div className="space-y-5">
              <div className="flex items-start gap-4 p-5 bg-slate-50 rounded-xl border border-slate-200">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-400 mt-1">{formatCurrency(viewingFee.amount)} â€” due {format(new Date(viewingFee.dueDate), 'MMMM dd, yyyy')}</p>
                </div>
              </div>
