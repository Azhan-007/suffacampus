...
export default function StudentNotificationsScreen() {
  ...
  const loadNotifications = async (isRefresh = false) => {
    try {
      ...
      const data = await getNotifications({ studentId, limit: 100 });
      setNotifications(data);

      const permission = await getPushPermissionState();
      setPermissionGranted(permission.granted);
    } catch (error) {
      console.warn("Error fetching student notifications:", error);
      setNotifications([]);
    } finally {
      setLoading(false);
      setRefreshing(false);
            ...
            {!permissionGranted && (
              <View style={styles.pushCard}>
                ...
                <View style={styles.pushCardContent}>
                  ...
                  <Text style={styles.pushCardText}>Get instant updates for assignments, attendance, and results.</Text>
                </View>
                <TouchableOpacity
